...
- `Npc`: identity (name, race, occupation, age band), `HomeId`, `WorkplaceId`, personality seed. Population (size-profile count) generated by the RegionGenerator pass and frozen into that world's `Saves/Worlds/{id}/world.json` — households of mixed size per the spec. (Corrected 2026-08-06: the old `starting_town.json` wording predates generated worlds; the region is content *inside* the world save, never a town-scoped file of its own — see rulings #7/#9 and Planning note 12.)
...
18. ~~**Underground (2026-08-05)** — v2, layer slot reserved in world.json now; mine stays Press-E in v1.~~ **Superseded 2026-08-06 by #36** for the mine itself; a separate underground *layer* remains v2.
...
32. **Damage flows from the systems (2026-08-05, IMPLEMENTED same day)** — attacker damage = weapon (or spell) x stat multiplier x weapon proficiency x elemental affinity x creature `SizeScale`. The flat `Attack` stats and the boss x1.35 + level-floor hardcoding are gone from the formula (superseding the 2026-07-24 boss-floor ruling — support-class bosses now honestly hit soft until real boss kits/creatures land). `SizeScale`: Basic 1.0 / Elite 1.1 / Boss 1.3; creature templates carry their own. Tuning knobs: primary +15%/pt, secondary +6%/pt in CombatSystem.
...
36. **Mining is destroying rock (2026-08-06, IMPLEMENTED same day)** — the mine is not a button you press at a wall: stone is destructible terrain that drops ore and gems. Digging happens **into the mountainside on the region map** (a separate underground layer stays v2), and **any stone can be broken** — mountainside, cave rock, town walls, the dungeon's own walls. Ore lies in **seams**, gems are rare finds within them, and seams thicken with depth. Two exceptions are bedrock and cannot be mined at any hardness: the map's rim, and the dungeon's threshold.
37. **The dungeon is a dimensional space (2026-08-06)** — it does not occupy physical volume inside the mountain; it is a separate space reached *through* the threshold. No tunnel dug into the mountainside can ever break into it, which is why an endless dungeon behind a finite mountainside is not a contradiction. Its threshold is correspondingly not ordinary stone and cannot be destroyed. Mining the dungeon's *interior* walls is permitted but **incredibly loud** — it draws attention, and **the dungeon itself may react to being damaged** (the reaction beyond noise is a recorded hook, not yet built).
